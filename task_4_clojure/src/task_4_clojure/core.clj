...
(defn notify-msg
  ...
  (if-not (contains? (:bill state) ware)
    state
    (let [bill   (:bill state)
          buffer (:buffer state)
          need   (max 0 (- (bill ware) (buffer ware 0)))
          take   (min need amount @storage-atom)
          taken  (try
                   (when (pos? take)
                     (swap! storage-atom #(do (- % take))))
                   take
                   (catch Exception _ 0))
          newbuf (if (pos? taken)
                   ...
                   buffer)
          ready? (fn [buf]
                   (every? (fn [[w n]] (>= (buf w 0) n)) bill))]
      (if-not (ready? newbuf)
        (assoc state :buffer newbuf)
        (let [buf-after (reduce-kv (fn [b w n] (update b w - n))
                                   newbuf bill)
              amount-produced (:amount state)
              duration        (:duration state)
              target-worker   (get-in state [:target-storage :worker])]
          (future
            (Thread/sleep duration)
            (send target-worker supply-msg amount-produced))
          (assoc state :buffer buf-after))))))
